...
type DelayingInterface interface {
	...
	AddAfterWithTrace(item interface{}, duration time.Duration, trace any)
}
...
func (q *delayingType) AddAfter(item interface{}, duration time.Duration) {
	q.AddAfterWithTrace(item, duration, nil)
}

func (q *delayingType) AddAfterWithTrace(item interface{}, duration time.Duration, trace any) {
	...
	case q.waitingForAddCh <- &waitFor{data: item, readyAt: q.clock.Now().Add(duration), trace: trace}:
	}
}
...
func (q *delayingType) waitingLoop() {
	...
	for {
		...
		for waitingForQueue.Len() > 0 {
			...
			q.AddWithTrace(entry.data, entry.trace)
		...
		case waitEntry := <-q.waitingForAddCh:
			if waitEntry.readyAt.After(q.clock.Now()) {
				...
				q.AddWithTrace(waitEntry.data, waitEntry.trace)
			}
