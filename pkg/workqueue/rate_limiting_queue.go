...
type RateLimitingInterface interface {
	...
	AddRateLimitedWithTrace(item interface{}, trace any)
...
func NewRateLimitingQueue(rateLimiter kworkqueue.RateLimiter) RateLimitingInterface {
...
// Remember to call Forget!  If you don't, you may end up tracking failures forever.
func NewRateLimitingQueueWithConfig(rateLimiter kworkqueue.RateLimiter, config RateLimitingQueueConfig) RateLimitingInterface {
	if config.Clock == nil {
...
func NewNamedRateLimitingQueue(rateLimiter kworkqueue.RateLimiter, name string) RateLimitingInterface {
	return NewRateLimitingQueueWithConfig(rateLimiter, RateLimitingQueueConfig{
		Name: name,
	})
}
...
// Deprecated: Use NewRateLimitingQueueWithConfig instead.
func NewRateLimitingQueueWithDelayingInterface(di DelayingInterface, rateLimiter kworkqueue.RateLimiter) RateLimitingInterface {
	return NewRateLimitingQueueWithConfig(rateLimiter, RateLimitingQueueConfig{
		DelayingQueue: di,
	})
}
...
func (q *rateLimitingType) AddRateLimited(item interface{}) {
	q.AddRateLimitedWithTrace(item, nil)
}

func (q *rateLimitingType) AddRateLimitedWithTrace(item interface{}, trace any) {
	q.DelayingInterface.AddAfterWithTrace(item, q.rateLimiter.When(item), trace)
}
